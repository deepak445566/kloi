...
function Addproduct() {
  ...
  const [shippingCharge, setShippingCharge] = useState("0"); // ✅ NEW
  const [freeShipping, setFreeShipping] = useState(false); // ✅ NEW
  ...
  const handlefile = async (e) => {
    try {
      ...
      const productData = {
        ...
        shippingCharge: freeShipping ? 0 : Number(shippingCharge), // ✅ NEW
        freeShipping // ✅ NEW
      };
    ...
    <>
      <div className=" no-scrollbar flex-1 h-[95vh] overflow-y-scroll flex flex-col justify-between">
        <form onSubmit={handlefile} className="md:p-10 p-4 space-y-5 max-w-lg">
          ...
          <div className="space-y-3">
            <div className="flex items-center gap-3">
              <input 
                ...
                className="w-5 h-5"
              />
              <label htmlFor="free-shipping" className="text-base font-medium cursor-pointer">
                Free Shipping
              </label>
            </div>
            ...
            {!freeShipping && (
              <div className="flex flex-col gap-1 max-w-md">
                <label className="text-base font-medium" htmlFor="shipping-charge">
                  Shipping Charge (₹) *
                </label>
                <input 
                ...
                <p className="text-sm text-gray-500 mt-1">
                  This charge will be applied per product in the cart
                </p>
              </div>
            )}
          </div>
          ...
          {/* Price Preview with GST and Shipping */}
              ...
              <div className="space-y-1 text-sm">
                ...
                <p className="flex justify-between font-medium border-t pt-2">
                  ...
                  <span>
                    ₹{(
                      Number(offerPrice) + 
                      (Number(offerPrice) * Number(gstPercentage) / 100) + 
                      (freeShipping ? 0 : Number(shippingCharge))
                    ).toFixed(2)}
                  </span>
                </p>
              </div>
