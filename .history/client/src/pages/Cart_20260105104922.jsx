...
const Cart = () => {
  ...
  const [paymentMethod, setPaymentMethod] = useState("online"); // ✅ Added payment method
  ...
  const placeOrder = async () => {
    ...
    try {
      const orderData = {
        ...
        paymentType: paymentMethod === "online" ? "Online" : "COD"
      };

      // Add transactionId only for online payment
      if (paymentMethod === "online") {
      ...
      <div className="max-w-[360px] w-full bg-gray-100/40 p-5 max-md:mt-16 border border-gray-300/70">
        ...
        <div className="mb-6">
            ...
            <div className="mt-4">
              ...
              <div className="space-y-2">
                ...
                <div className="text-xs text-gray-500 space-y-1">
                  <p>⚠️ Enter the transaction ID from your payment confirmation</p>
                  <p>💡 Example: UPI Transaction ID, Payment Reference Number</p>
                  <p>✅ WhatsApp confirmation will be sent to seller automatically</p>
                </div>
              </div>
            </div>
          )}
          ...
          {paymentMethod === "online" && (
            <div className="mt-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
              ...
              <ol className="text-sm text-blue-700 list-decimal pl-5 space-y-1">
                <li>Pay using UPI/Net Banking to our account</li>
                <li>Enter the Transaction ID above</li>
                <li>Click "Place Order"</li>
                <li>Seller will get WhatsApp notification immediately</li>
              </ol>
              ...
              <div className="mt-4 text-center">
                ...
                />
                <p className="text-xs text-gray-600 mt-2">Scan to pay via UPI</p>
              </div>
            </div>
          )}
          ...
          {paymentMethod === "cod" && (
            <div className="mt-6 p-4 bg-green-50 border border-green-200 rounded-lg">
              <p className="font-medium text-green-800 mb-2">💰 Cash on Delivery</p>
              <ul className="text-sm text-green-700 space-y-1">
                <li>✅ Pay when you receive the order</li>
                <li>✅ No online payment required</li>
                <li>✅ WhatsApp notification will still be sent to seller</li>
                <li>✅ You'll receive order confirmation email/SMS</li>
              </ul>
            </div>
          )}
        </div>
        ...
        <div className="text-gray-500 mt-4 space-y-2">
          ...
          <p className="flex justify-between text-lg font-medium mt-3">
            ...
            <span>₹{gettotal() + Math.floor(gettotal() * 0.05)}</span>
          </p>
        </div>

        <button
          onClick={placeOrder}
          disabled={isPlacingOrder || (paymentMethod === "online" && (!transactionId || transactionError))}
          className={`w-full py-3 mt-4 cursor-pointer ${
            isPlacingOrder || (paymentMethod === "online" && (!transactionId || transactionError)) 
            ? 'bg-gray-400' 
        ...
        <p className="text-xs text-center text-gray-500 mt-3">
          ✅ Seller will receive WhatsApp notification for your order
        </p>
      </div>
    </div>
  ) : null
