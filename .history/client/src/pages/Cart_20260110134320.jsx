...
import { Truck, Package, ShoppingBag, CreditCard, MapPin, User, CheckCircle, MessageCircle, Wallet } from 'lucide-react';
...
const Cart = () => {
  ...
  const validateTransactionId = (id) => {
    if (paymentMethod === 'cod' && (!id || id.trim() === '')) {
      return "Transaction ID is required for COD orders";
    }
    if (paymentMethod === 'cod' && id.trim().length < 8) {
  ...
  const generateWhatsAppMessage = (orderData) => {
    const { orderId, totalAmount, transactionId, customerPhone, paymentType } = orderData;
...
_This is an automated order notification. Please process the order._`;
  ...
  const sendWhatsAppNotification = (orderId, totalAmount, transactionId, paymentType) => {
    ...
    <div className="flex flex-col lg:flex-row gap-8 mt-12">
      ...
      <div className="lg:w-96">
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6 sticky top-6">
          ...
          {/* Transaction ID Input (Only for COD) */}
          {paymentMethod === 'cod' && (
            <div className="mb-6">
          ...
          <button
            ...
            disabled={
              ...
              razorpayLoading || 
              !selectedAddress || 
              (paymentMethod === 'cod' && (!transactionId || transactionError))
            }
            className={`w-full py-4 mt-4 rounded-lg font-bold text-lg transition-all ${
              isPlacingOrder || razorpayLoading || !selectedAddress || (paymentMethod === 'cod' && (!transactionId || transactionError))
          ...
          >
            {isPlacingOrder || razorpayLoading ? (
              <>
                ...
                {paymentMethod === 'razorpay' ? 'Processing Payment...' : 'Processing & Sending WhatsApp...'}
              </>
          ...
          {paymentMethod === 'cod' && !transactionId && (
            <p className="text-xs text-red-500 mt-2 text-center">
              Please enter transaction ID for COD order
            </p>
          )}
          ...
          {paymentMethod === 'razorpay' && (
            <div className="mt-4 text-center">
              <div className="flex items-center justify-center gap-2 text-sm text-gray-600">
                <img src="https://razorpay.com/assets/razorpay-glyph.svg" alt="Razorpay" className="w-4 h-4" />
                <span>100% Secure Payment by Razorpay</span>
              </div>
              <div className="flex justify-center gap-4 mt-2">
                <img src="https://razorpay.com/assets/payment-methods/upi.svg" alt="UPI" className="h-6" />
                <img src="https://razorpay.com/assets/payment-methods/cards.svg" alt="Cards" className="h-6" />
                <img src="https://razorpay.com/assets/payment-methods/netbanking.svg" alt="NetBanking" className="h-6" />
              </div>
            </div>
          )}
        </div>
