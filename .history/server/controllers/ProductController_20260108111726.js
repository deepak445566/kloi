...
export const updateProduct = async (req, res) => {
  try {
    ...
    // ✅ Ensure weightUnit is included
    if (!updateData.weightUnit) {
      updateData.weightUnit = "kg";
    }

    // ✅ Ensure weightValue is converted to number
    if (updateData.weightValue) {
      updateData.weightValue = Number(updateData.weightValue);
    }

    // ✅ Ensure GST percentage is included
    if (!updateData.gstPercentage) {
      updateData.gstPercentage = 5; // Default 5%
    }
